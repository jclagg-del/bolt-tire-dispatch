...
type Product = {
  ...
  warranty: string; cost?: number; quotePrice: number; installedPrice: number; imageUrl: string | null; snowRated: boolean;
    ...
    <div className="tire-beta-layout">
      ...
      <section className="tire-beta-results"><div className="tire-beta-result-head"><div><strong>{results.length} tires</strong><span>{searched ? " from ATD sandbox" : " — enter a size above"}</span></div><select value={sort} onChange={(event) => setSort(event.target.value)}><option value="price">Lowest installed price</option><option value="availability">Best availability</option>{internal && <option value="margin">Best gross profit</option>}</select></div>
        ...
        {!searched || loading || results.length === 0 ? <div className="tire-beta-empty"><strong>{loading ? "Searching ATD…" : searched ? "No available products matched." : "Search ATD by raw tire size."}</strong><span>{searched && !loading ? "Try turning off Available nearby or changing the size." : "Live sandbox results will appear here."}</span></div> : results.map((tire) => { const isSelected=selected.some((item)=>item.id===tire.id); return <article className={`tire-beta-product ${isSelected ? "selected" : ""}`} key={tire.id}>
          ...
          <div className="tire-beta-price">{internal && <><span>ATD cost <strong>${(tire.cost || 0).toFixed(2)}</strong></span><span>Gross profit <strong>${(tire.quotePrice - (tire.cost || 0)).toFixed(2)}</strong></span><small>{tire.atdProductNumber}</small></>}<strong className="tire-beta-installed">${tire.installedPrice.toFixed(2)}</strong><span>estimated installed per tire</span><button className={isSelected?"selected":""} type="button" disabled={internal&&!isSelected&&selected.length>=3} onClick={()=>internal?toggleSelected(tire):undefined}>{internal ? isSelected ? "✓ Selected" : selected.length>=3 ? "3 selected" : "Select for quote" : "Choose tire"}</button></div>
        </article>})}
      </section>
    </div>
  </main>;
