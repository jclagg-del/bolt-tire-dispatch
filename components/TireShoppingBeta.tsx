...
type Product = {
  ...
  loadRange: string; discontinued: boolean; runFlat:boolean; hasRebate:boolean; rebates?:Array<{code:string;description:string;url:string}>; serviceCategory:"passenger"|"truck"; fitmentPosition:"front"|"rear"|"both"; atdProductNumber: string;
...
export default function TireShoppingBeta({ internal = false }: { internal?: boolean }) {
  ...
  const [mode,setMode]=useState<"size"|"vehicle">(internal?"size":"vehicle");
  ...
  useEffect(()=>{if(!internal&&!years.length)loadFitment("years");},[internal]);
  ...
  const staggered=results.some(item=>item.fitmentPosition==="front")&&results.some(item=>item.fitmentPosition==="rear");
  function openPurchaseBuilder(items:Product[]){sessionStorage.setItem("bolt-tire-purchase",JSON.stringify({query,products:items,quantity:items.length===2?4:quantity}));router.push("/shop/configure")}

  return <main className={`tire-beta-page ${internal?"internal":"public"}`}>
    <header className="tire-beta-hero"><div><span className="tire-beta-kicker">{internal ? "Bolt Tire staff" : "Tires brought to you"}</span><h1>{internal ? "Tire Search & Quoting" : "The right tires. Installed where you are."}</h1><p>{internal?"Live ATD products, pricing, images, and inventory.":"Shop by vehicle or tire size, compare transparent pricing, and request mobile installation from Bolt Tire."}</p>{!internal&&<div className="tire-beta-benefits"><span>✓ Mobile installation</span><span>✓ Upfront estimates</span><span>✓ Local availability</span></div>}</div>{internal?<span className="tire-beta-badge">ATD live</span>:<img className="tire-beta-hero-logo" src="/bolt-logo-white.png" alt="Bolt Tire" />}</header>
    <section className="tire-beta-search-card">
      <div className="tire-beta-tabs">{!internal&&<button className={mode==="vehicle"?"active":""} type="button" onClick={()=>{setMode("vehicle");if(!years.length)loadFitment("years")}}>Shop by vehicle</button>}<button className={mode==="size"?"active":""} type="button" onClick={()=>setMode("size")}>Shop by tire size</button>{internal&&<button className={mode==="vehicle"?"active":""} type="button" onClick={()=>{setMode("vehicle");if(!years.length)loadFitment("years")}}>Vehicle fitment</button>}{internal && <button type="button" disabled>VIN / plate · coming next</button>}</div>
    ...
    {error && <div className="tire-beta-error"><strong>{internal?"ATD connection error":"Tire search is temporarily unavailable"}</strong><span>{internal?error:"Please try again in a moment. If this continues, call or text Bolt Tire for help."}</span></div>}
    <div className="tire-beta-layout">
      ...
      <section className="tire-beta-results"><div className="tire-beta-result-head"><div><strong>{results.length} tires</strong><span>{searched ? " from live inventory" : " — enter a size above"}</span></div><div className="tire-beta-result-controls"><label>Quantity <select value={quantity} onChange={e=>setQuantity(Number(e.target.value))}>{[1,2,3,4,5,6].map(item=><option key={item} value={item}>{item}</option>)}</select></label><select aria-label="Sort tires" value={sort} onChange={(event) => setSort(event.target.value)}><option value="price">Lowest installed price</option><option value="availability">Best availability</option>{internal && <option value="margin">Best gross profit</option>}</select></div></div>
        {internal && searched && <div className="tire-beta-quote-tray"><div><strong>{selected.length} of 3 selected</strong><span>Choose up to three tires to compare on the quote.</span></div><button type="button" disabled={!selected.length} onClick={buildQuote}>Generate Quote</button></div>}
        {staggered&&<div className="tire-beta-staggered-note"><strong>Staggered fitment</strong><span>Choose one front tire and one rear tire. Your quote will include two of each.</span></div>}
        {!searched || loading || results.length === 0 ? <div className="tire-beta-empty"><strong>{loading ? "Searching ATD…" : searched ? "No available products matched." : "Search ATD by raw tire size."}</strong><span>{searched && !loading ? "Try turning off Available nearby or changing the size." : "Live inventory results will appear here."}</span></div> : results.map((tire) => { const isSelected=selected.some((item)=>item.id===tire.id); return <article className={`tire-beta-product ${isSelected ? "selected" : ""}`} key={tire.id}>
          ...
          <div className="tire-beta-product-main"><div className="tire-beta-brand-row"><span className="tire-beta-brand">{tire.brand}</span>{tire.fitmentPosition!=="both"&&<span className={`tire-beta-position ${tire.fitmentPosition}`}>{tire.fitmentPosition} fitment</span>}</div><h2>{tire.model}</h2><strong>{tire.size || tire.description}{tire.loadSpeed ? ` · ${tire.loadSpeed}` : ""}</strong><div className="tire-beta-tags"><span>{tire.category}</span>{tire.warranty && <span>{tire.warranty} warranty</span>}{tire.loadRange && <span>Load {tire.loadRange}</span>}{tire.snowRated && <span>3PMSF</span>}{tire.runFlat&&<span>Run-flat</span>}{tire.hasRebate&&<span className="rebate-tag">Rebate available</span>}{tire.discontinued && <span>Discontinued</span>}</div>{tire.rebates?.length?<div className="tire-beta-rebates">{tire.rebates.map((rebate,index)=><div key={`${rebate.code}-${index}`}><strong>{rebate.description}</strong>{rebate.url?<a href={rebate.url} target="_blank" rel="noreferrer">View rebate details</a>:<small>Manufacturer terms apply</small>}</div>)}</div>:null}<p>{tire.availability.local ? "Available locally" : tire.availability.localPlus ? internal ? "Available from Local Plus" : "Available nearby — typically 1–2 days" : internal ? "Special order" : "Available to order"}</p>{internal && <div className="tire-beta-stock"><span>Local <strong>{tire.availability.local}</strong></span><span>Local Plus <strong>{tire.availability.localPlus}</strong></span><span>Nationwide <strong>{tire.availability.nationwide}</strong></span></div>}</div>
          <div className="tire-beta-price">{internal && <><span>ATD cost <strong>${(tire.cost || 0).toFixed(2)}</strong></span><span>Gross profit <strong>${(tire.quotePrice - (tire.cost || 0)).toFixed(2)}</strong></span><small>{tire.atdProductNumber}</small></>}<div className="tire-beta-customer-price"><span>Tire price</span><strong>${tire.quotePrice.toFixed(2)} <small>each</small></strong></div>{!staggered&&<div className="tire-beta-total-price"><span>Estimated total for {quantity}</span><strong>${Number(tire.estimatedTotals?.[String(quantity)]??tire.installedPrice*quantity).toFixed(2)}</strong><small>tires, installation & standard fees</small></div>}<button className={isSelected?"selected":""} type="button" disabled={internal&&!isSelected&&selected.length>=3} onClick={()=>{if(internal)return toggleSelected(tire);if(!staggered)return openPurchaseBuilder([tire]);const next=isSelected?selected.filter(item=>item.id!==tire.id||item.fitmentPosition!==tire.fitmentPosition):[...selected.filter(item=>item.fitmentPosition!==tire.fitmentPosition),tire];setSelected(next);if(next.some(item=>item.fitmentPosition==="front")&&next.some(item=>item.fitmentPosition==="rear"))openPurchaseBuilder(next)}}>{internal ? isSelected ? "✓ Selected" : selected.length>=3 ? "3 selected" : "Select for quote" : staggered?`Choose ${tire.fitmentPosition}`:"Customize & buy"}</button></div>
        </article>})}
      </section>
    </div>
  </main>;
