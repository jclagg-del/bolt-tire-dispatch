...
type Product = {
  ...
  loadRange: string; discontinued: boolean; runFlat:boolean; hasRebate:boolean; serviceCategory:"passenger"|"truck"; atdProductNumber: string;
...
export default function TireShoppingBeta({ internal = false }: { internal?: boolean }) {
  ...
  const [customer,setCustomer]=useState({name:"",phone:"",email:"",vehicle:"",address:""});
  ...
  async function createPublicQuote(){const tire=selected[0];if(!tire)return;setSubmitting(true);setError("");try{const response=await fetch("/api/public/shop/quote",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({...customer,query,productId:tire.id,quantity})});const payload=await response.json();if(!response.ok)throw new Error(payload.error||"Could not create quote");router.push(`/q/${payload.token}`);}catch(reason){setError(reason instanceof Error?reason.message:"Could not create quote");setSubmitting(false);}}
  ...
  return <main className="tire-beta-page">
    ...
    <div className="tire-beta-layout">
      ...
      <section className="tire-beta-results"><div className="tire-beta-result-head"><div><strong>{results.length} tires</strong><span>{searched ? " from ATD sandbox" : " — enter a size above"}</span></div><div className="tire-beta-result-controls"><label>Quantity <select value={quantity} onChange={e=>setQuantity(Number(e.target.value))}>{[1,2,3,4,5,6].map(item=><option key={item} value={item}>{item}</option>)}</select></label><select aria-label="Sort tires" value={sort} onChange={(event) => setSort(event.target.value)}><option value="price">Lowest installed price</option><option value="availability">Best availability</option>{internal && <option value="margin">Best gross profit</option>}</select></div></div>
        ...
        {!internal&&selected[0]&&<div className="tire-beta-customer-form"><div><strong>Request this tire</strong><span>{selected[0].brand} {selected[0].model} · {quantity} tire{quantity===1?"":"s"}</span></div><div className="tire-beta-customer-grid"><label>Name *<input value={customer.name} onChange={e=>setCustomer({...customer,name:e.target.value})}/></label><label>Phone<input type="tel" value={customer.phone} onChange={e=>setCustomer({...customer,phone:e.target.value})}/></label><label>Email<input type="email" value={customer.email} onChange={e=>setCustomer({...customer,email:e.target.value})}/></label><label>Vehicle<input placeholder="Year, make and model" value={customer.vehicle} onChange={e=>setCustomer({...customer,vehicle:e.target.value})}/></label><label className="full">Service address<input value={customer.address} onChange={e=>setCustomer({...customer,address:e.target.value})}/></label></div><small>Enter a phone number or email so Bolt Tire can confirm availability and scheduling.</small><button type="button" disabled={submitting||!customer.name.trim()||(!customer.phone.trim()&&!customer.email.trim())} onClick={createPublicQuote}>{submitting?"Creating quote…":"Continue to Quote"}</button></div>}
