...
export default function KingdomOrdersPage() {
  ...
  const visible = useMemo(() => orders.filter(order => {
    ...
    const haystack = [order.job_number, order.mo_number, order.service_method, order.vehicle_year, order.vehicle_make, order.vehicle_model, order.vehicle_color, order.tire_size, order.tire_product_number, s].filter(Boolean).join(" ").toLowerCase();
