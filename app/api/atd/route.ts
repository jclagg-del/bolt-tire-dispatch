...
import { fitmentList, searchAtdByFitment, searchAtdBySize } from "@/lib/atd";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.action === "size") return NextResponse.json({ products: await searchAtdBySize(String(body.query || ""), includeCost), sandbox: true });
    if (body.action === "fitment-products") return NextResponse.json({ products: await searchAtdByFitment(body.vehicle || {}, includeCost), sandbox: true });
