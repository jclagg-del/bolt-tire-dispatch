...
import { getConnection, quickBooksConfig } from "@/lib/quickbooks";

export async function GET(request: Request) {
  try {
    if (!(await requireApiUser(request))) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    const connection = await getConnection();
    return NextResponse.json({ connected: true, environment: quickBooksConfig().environment, expiresAt: connection.access_expires_at });
  } catch (error) {
