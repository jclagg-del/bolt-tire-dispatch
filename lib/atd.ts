...
function presentProducts(products: AtdProduct[], inventory: Map<string, InventoryProduct>, includeCost: boolean, settings: BusinessSettings) {
  return products.filter((product) => !product.replaced).map((product) => {
    ...
    return {
      ...
      treadDepth: product.productspec?.treaddepth || product.productspec?.tiretreaddepth || "",
      utqg: product.productspec?.utqg || "",
      sidewall: product.productspec?.sidewall || product.productspec?.sidewalldescription || "",
      maxLoad: product.productspec?.maxload || product.productspec?.maxloadsingle || "",
      rimRange: product.productspec?.approvedrimwidth || product.productspec?.rimwidthrange || "",
      oeMarking: product.productspec?.oedesignation || product.productspec?.oemarking || "",
